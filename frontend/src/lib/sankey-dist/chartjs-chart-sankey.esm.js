...
const startColumn = (data, nodes) => {
    ...
    for (const point of data) {
        if (
            !referencedNodes.has(point.from) &&
            !referencedNodes.has(point.to)
        ) {
...
const nextColumn = (dataWithoutDirectLoops, remainingKeys) => {
    const remainingTo = new Set(
        dataWithoutDirectLoops
            .filter((flow) => remainingKeys.has(flow.from))
            .map((flow) => flow.to)
    );
    const remainingKeyArray = [...remainingKeys];
    const columnsNotInTo = remainingKeyArray.filter(
        (key) => !remainingTo.has(key)
    );
    return columnsNotInTo.length
        ? columnsNotInTo
        : remainingKeyArray.slice(0, 1);
};
function calculateX(nodeMap, data, mode) {
    ...
    while (keysToPlace.size) {
        const column =
            x === 0
                ? startColumn(data, allNodes)
                : nextColumn(dataWithoutDirectLoops, keysToPlace);
        if (!column.length) {
            throw new Error(
                "Fatal error: Unable to place nodes to columns. Please report this issue."
            );
        }
...
function nodeCount(list, prop, countId = getCountId()) {
    ...
    for (const elem of list) {
        ...
        count +=
            elem.node[prop].length + nodeCount(elem.node[prop], prop, countId);
    }
    return count;
}
...
function processRest(nodeArray, maxX) {
    ...
    const centerToDo = nodeArray.filter(
        (node) => node.x > 0 && node.x < maxX && !defined(node.y)
    );
    let leftY =
        leftNodes.reduce((acc, cur) => Math.max(acc, cur.y + cur.out || 0), 0) +
        SMALL_VALUE;
    let rightY =
        rightNodes.reduce((acc, cur) => Math.max(acc, cur.y + cur.in || 0), 0) +
        SMALL_VALUE;
    let centerY = 0;
    if (leftY >= rightY) {
    ...
    centerToDo.forEach((node) => {
        let y = nodeArray
            ...
            .reduce(
                (acc, cur) => Math.max(acc, cur.y + Math.max(cur.in, cur.out)),
                0
            );
        y = setOrGetY(node, y);
...
function sortFlows(nodeArray) {
    nodeArray.forEach((node) => {
        ...
        node.from
            .sort(
                (a, b) =>
                    a.node.y + a.node.out / 2 - (b.node.y + b.node.out / 2)
            )
            .forEach((flow, idx) => {
                if (overlapFrom) {
        ...
        node.to
            .sort(
                (a, b) => a.node.y + a.node.in / 2 - (b.node.y + b.node.in / 2)
            )
            .forEach((flow, idx) => {
                if (overlapTo) {
                    flow.addY = (idx * (nodeSize - flow.flow)) / (len - 1);
...
function layout(nodes, data, { priority, height, nodePadding, modeX }) {
    ...
    const maxY = priority
        ? calculateYUsingPriority(nodeArray, maxX)
        : calculateY(nodeArray, maxX);
...
class SankeyController extends DatasetController {
    parseObjectData(meta, data, start, count) {
        ...
        const parsed = [];
        const nodes = (this._nodes = buildNodesFromData(
            sankeyData,
            this.options
        ));
...
const applyAlpha = (original, alpha) =>
    color(original).alpha(alpha).rgbString();
const getColorOption = (option, alpha) =>
...
function setStyle(ctx, { x, x2, options }) {
    ...
    if (options.colorMode === "from") {
        ...
        typeof options.colorFrom === "string" &&
        typeof options.colorTo === "string"
    ) {
